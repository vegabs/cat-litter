...
class Auth:
    ...
    def sign_in_with_email_and_password(self, email, password):
        ...
        mdata = json.dumps({"email": email, "password": password, "returnSecureToken": True})
        request_object = self.requests.post(request_ref, headers=mheaders, data=mdata)
...
class Database:
    ...
    def build_headers(self, token=None):
        ...
        if not token and self.credentials:
            access_token = self.credentials["idToken"]

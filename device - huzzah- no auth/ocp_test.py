...
class OCPTest:
    ...
    def __init__(self,pstat,param):
        pyrebase.db.set("begin ocp test")
        pyrebase.db.push("begin ocp test")
        self.param = {}
        self.done = False
        self.pstat = pstat
        self.t_start = 0.0
    ...
    def update(self):
        ...
        if (self.t_now >= self.t_next_sample) or self.done:
            ...
            pyrebase.test_data.append({'t': self.elapsed_time, 'v': random.uniform(0.055,0.069), 'i': current_uA})

            if self.done:
                pyrebase.test_data[-1]["done"]=True
                pyrebase.write_wifi(pyrebase.test_data)
                send_to_self({'pump':3.0})
            if (len(pyrebase.test_data) >= 16):
